...
def test_eod_mode_when_no_chains_provided():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert result.mode == EngineMode.EOD_GENERAL
        assert result.verdict == Verdict.GENERAL_STRATEGY
        assert result.primary_strategy_family in (StrategyFamily.BULL_PUT_CREDIT, StrategyFamily.BULL_CALL_DEBIT)
        assert result.requires_chain_validation is True
        assert "strikes" in result.unavailable_fields
        assert "bid_ask_spreads" in result.unavailable_fields


def test_full_dde_mode_with_valid_option_chain():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert result.verdict in (Verdict.TRADE, Verdict.WATCH, Verdict.PASS)
