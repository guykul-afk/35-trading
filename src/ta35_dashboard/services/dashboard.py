...
ללא בחינת הרמות עצמן אין להבדיל ביניהם בוודאות.""",
    ...
    (
        ...
        "שיפוע עקום VIX",
        ".2f",
        """**מהו המדד?** שיפוע עקום התנודתיות הגלובלי (VIX9D/VIX פחות VIX/VIX3M).

**איך הוא בנוי?** הפער בין היחס הקצר מאוד (9 ימים מול חודש) ליחס החודשי מול רבעוני.

**איך מפרשים?** ערך חיובי מסמן עקום הפוך ולחץ מיידי בעולם; ערך שלילי מסמן עקום רגוע (Contango).

**חשוב לדעת:** שיפוע חיובי חד מקדים התפרצות תנודתיות במניות.""",
    ),
)
...
RESEARCH_CARD_DEFINITIONS = (
    ...
    (
        ...
        ".2%",
        _research_help(
            "עוצמה יחסית של סקטור הבנקים מול מדד ת״א־35.",
            "מומנטום התשואה היחסית של מדד ת״א-בנקים 5 מול ת״א-35 ביחס לממוצע 20 יום.",
            "ערך חיובי מציין הובלה מוסדית חיובית של הבנקים; ערך שלילי מציין חולשה סקטוריאלית.",
            "הבנקים מהווים מעל 35% מהמדד ומובילים תנועות שוק; אינדיקטור כיווני מוביל.",
        ),
    ),
    (
        "credit_spread_stress",
        "מרווח אשראי 60/ממשלתי",
        ".2f",
        _research_help(
            "מרווח אשראי קונצרני (תל-בונד 60) מול אג״ח ממשלתי.",
            "ציון תקן z-score של יחס המחירים תל-בונד 60 לתל גוב ביחס ל־20 יום.",
            "ערך שלילי מצביע על התרחבות מרווחי אשראי והידוק נזילות; מקדים עלייה ב־VTA35.",
            "אינדיקטור אשראי מקדים, דורש הזנת תל-בונד 60 ותל גוב-כללי.",
        ),
    ),
    (
        "flight_to_safety",
        "מנוסה למקלט מניות/אג״ח",
        ".2f",
        _research_help(
            "זרימת כספים יחסית בין מניות (ת״א-35) לאג״ח ממשלתי.",
            "פער תשואת 5 ימים בין ת״א-35 לתל גוב-כללי מנורמל לסטיית התקן של המניות.",
            "חיובי מסמן מצב Risk-On ואופטימיות; שלילי מסמן מעבר לנכסי מקלט (De-risking).",
            "משקף תנועות הון מוסדיות בין אפיקים.",
        ),
    ),
    (
        "yield_curve_slope",
        ...
        ".2%",
        _research_help(
            "שיפוע עקום האג״ח הממשלתי (10+ מול קצר/כללי).",
            "מומנטום פער התשואה היחסי של תל גוב 10+ מול תל גוב 2Y/כללי ביחס ל־20 יום.",
            "עקום תלול תומך ברווחיות הבנקים ובצמיחה; השתטחות מסמנת האטה וריסון מוניטרי.",
            "אינדיקטור מקרו התומך בחיזוי כיוון המניות והבנקים.",
        ),
    ),
    (
        "banks_momentum_5d",
        "מומנטום בנקים 5 ימים",
        ".1%",
        _research_help(
            "קצב השינוי השבועי במדד ת״א בנקים-5.",
            "תשואת מדד הבנקים בחמשת ימי המסחר האחרונים.",
            "חיובי מסמן מומנטום סקטוריאלי חיובי של הסקטור המוביל בבורסה.",
            "מומנטום קצר טווח המשמש לאימות כיוון ת״א-35.",
        ),
    ),
    (
        "banks_ta35_corr_20",
        "קורלציה בנקים/ת״א-35",
        ".2f",
        _research_help(
            "מתאם מתגלגל 20 יום בין תשואות הבנקים לת״א-35.",
            "מקדם מתאם פירסון של תשואות יומיות ב־20 הימים האחרונים.",
            "מעל 0.70 מעיד על שוק אחיד ורוחבי; מתחת ל־0.40 מעיד על פיצול בין הסקטורים.",
            "קורלציה גבוהה מעצימה את השפעת הבנקים על המדד.",
        ),
    ),
    (
        "gov_bond_momentum_5d",
        "מומנטום אג״ח ממשלתי 5 ימים",
        ".1%",
        _research_help(
            "קצב השינוי השבועי במדד תל גוב-כללי.",
            "תשואת תל גוב-כללי בחמשת ימי המסחר האחרונים.",
            "עלייה חדה במחירי האג״ח מעידה על ירידת תשואות וסביבת ריבית נוחה.",
            "ירידה חדה במחירי האג״ח מאותתת על לחץ תשואות המכביד על מניות.",
        ),
    ),
    (
        "stock_bond_corr_20",
        "קורלציה מניות/אג״ח ממשלתי",
        ".2f",
        _research_help(
            "מתאם מתגלגל 20 יום בין תשואות ת״א-35 לתל גוב-כללי.",
            "מקדם מתאם של 20 תשואות יומיות בין מניות לאג״ח ממשלתי.",
            "ערך שלילי מסמן פיזור ומקלט קלאסי (בריחה מסיכון); ערך חיובי מסמן משטר ריבית/אינפלציה שבו שניהם נעים יחד.",
            "מגדיר את משטר הנזילות והמאקרו בשוק.",
        ),
    ),
    (
        "credit_bond_momentum_5d",
        "מומנטום תל-בונד 60 (5 ימים)",
        ".1%",
        _research_help(
            "קצב השינוי השבועי במדד תל-בונד 60.",
            "תשואת תל-בונד 60 בחמשת ימי המסחר האחרונים.",
            "חיובי מסמן נזילות ויציבות בשוק האשראי של החברות.",
            "ירידה מאותתת על התייקרות עלויות גיוס והידוק אשראי.",
        ),
    ),
)
